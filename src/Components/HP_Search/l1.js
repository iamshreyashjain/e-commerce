...
export default function HP_Search() {
    ...
    const [categoryMF, setCategoryMF] = useState({
        Brand: [],
        Ram: [],
        InternalStorage: []
    });
    ...
    const handleOnChecked = (event, category) => {
        const { value, checked } = event.target;
        setCategoryMF((prevCategoryMF) => {
            const newCategoryMF = { ...prevCategoryMF };
            if (checked) {
                newCategoryMF[category] = [...newCategoryMF[category], value];
            } else {
                newCategoryMF[category] = newCategoryMF[category].filter(item => item !== value);
            }
            return newCategoryMF;
        });
    };
                    ...
                    <div>
                        <div className="accordion" id="sortAccordion">
                            <div className="accordion-item overflow-hidden">
                                ...
                                <div id="sortCollapse" className="accordion-collapse collapse show" data-bs-parent="#sortAccordion">
                                            ...
                                            {category.specifications.map((spec, specIndex) => (
                                                <div className="form-check" key={specIndex}>
                                                    <input 
                                                        ...
                                                        value={spec}
                                                        checked={categoryMF[category.subCategory].includes(spec)}
                                                        onChange={(e) => handleOnChecked(e, category.subCategory)} 
                                                    />
